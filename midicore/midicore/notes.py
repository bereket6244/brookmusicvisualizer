"""Pitch/name helpers and General MIDI program names."""

# Sharps-only spelling. Pitch class 0 == C.
_NOTE_NAMES = ["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"]

MIDDLE_C = 60  # Convention used throughout this project: MIDI pitch 60 == C4.


def pitch_to_name_octave(pitch: int) -> tuple[str, int]:
    """Return (note_name, octave) for a MIDI pitch number.

    Octave numbering follows the "C4 = middle C = 60" convention,
    so octave = pitch // 12 - 1.
    """
    return _NOTE_NAMES[pitch % 12], pitch // 12 - 1


def pitch_to_full_name(pitch: int) -> str:
    name, octave = pitch_to_name_octave(pitch)
    return f"{name}{octave}"


# General MIDI level 1 program names, index 0..127 (program numbers as they
# appear in program_change messages, i.e. already zero-based).
GM_PROGRAM_NAMES = [
...
def program_name(program: int | None) -> str | None:
    if program is None:
        return None
    if 0 <= program < 128:
